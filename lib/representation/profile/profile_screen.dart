...
// MODEL DỮ LIỆU (Chuẩn bị cho Firebase)
class PostItem {
  final String id;
...
class _ProfileScreenState extends State<ProfileScreen> with SingleTickerProviderStateMixin {
  ...
  final List<PostItem> _feeds = List.generate(15, (index) => PostItem(
    id: 'feed_$index', 
    thumbnailUrl: 'https://placehold.co/300x300/png?text=Feed+$index', 
    type: 'image'
  ));

  final List<PostItem> _shorts = List.generate(10, (index) => PostItem(
    id: 'short_$index', 
                          ...
                          delegate: _SliverTabBarDelegate(
                            TabBar(
                              ...
                              tabs: const [
                                ...
                                Tab(icon: Icon(Icons.video_library_outlined), text: "Shorts"),
    ...
    return Builder(
      builder: (BuildContext context) {
        return CustomScrollView(
          key: PageStorageKey<String>(data.isNotEmpty ? data.first.type : 'empty'),
          slivers: [
  ...
  Widget _buildHeaderActions() {
    return Row(
      ...
      children: [
        ...
        _buildCircleIconBtn(Icons.more_vert),
      ],
    );
  }
  ...
  Widget _buildUserInfo() {
    return Column(
      children: [
        const Text(
          "Nguyễn Văn A",
          style: TextStyle(fontFamily: 'Inter', fontWeight: FontWeight.w500, fontSize: 18, color: Colors.black),
        ),
        const SizedBox(height: 4),
        const Text(
          "UI/UX Designer",
          style: TextStyle(fontFamily: 'Inter', fontWeight: FontWeight.w300, fontSize: 12, color: Colors.black87),
        ),
        const SizedBox(height: 4),
        Row(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            Transform.rotate(angle: -0.7, child: const Icon(Icons.link, size: 16, color: mainOrange)),
            const SizedBox(width: 4),
            const Text(
              "www.example.vn",
              style: TextStyle(fontFamily: 'Inter', fontWeight: FontWeight.w300, fontSize: 12, color: mainOrange),
            ),
          ],
        ),
      ],
        ...
        children: [
          ...
          Expanded(
            child: OutlinedButton(
              ...
              child: const Text("Message", style: TextStyle(color: mainOrange, fontWeight: FontWeight.w600)),
            ),
          ),
        ],
      ),
  ...
  Widget _buildStatItem(String number, String label) {
    return Column(
      children: [
        Text(number, style: const TextStyle(fontWeight: FontWeight.w500, fontSize: 16)),
        Text(label, style: const TextStyle(fontWeight: FontWeight.w300, fontSize: 11, color: greyText)),
      ],
    );
  }
